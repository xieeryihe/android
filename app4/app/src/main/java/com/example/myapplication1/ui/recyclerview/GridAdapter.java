...
public class GridAdapter extends RecyclerView.Adapter<GridAdapter.LinearViewHolder> {
    ...
    public GridAdapter(Context context){//写一个本Adapter的构造函数
        this.mContext = context;
    }
    public GridAdapter(Context context, OnItemClickListener listener){//重载的用于接受mListener传参的构造
        this.mContext = context;
    ...
    public GridAdapter.LinearViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        //其中layout_linear_item就是layout文件夹下的layout_grid_item.xml文件的文件名（注意不是id），传入的是布局文件
    }

    @Override
    public void onBindViewHolder(@NonNull GridAdapter.LinearViewHolder holder, final int position) {
        holder.textView.setText("Hello grid!");//这行代码多看一下几个变量的定义在哪就理解了
