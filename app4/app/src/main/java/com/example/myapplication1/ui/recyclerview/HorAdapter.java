...
public class HorAdapter extends RecyclerView.Adapter<HorAdapter.LinearViewHolder> {
    ...
    public HorAdapter(Context context){//写一个本Adapter的构造函数
        this.mContext = context;
    }
    public HorAdapter(Context context, OnItemClickListener listener){//重载的用于接受mListener传参的构造
        this.mContext = context;
    ...
    public HorAdapter.LinearViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        //其中layout_linear_item就是layout文件夹下的layout_hor_item.xml文件的文件名（注意不是id），传入的是布局文件
    }

    @Override
    public void onBindViewHolder(@NonNull HorAdapter.LinearViewHolder holder, final int position) {
        holder.textView.setText("Hello hor!");//这行代码多看一下几个变量的定义在哪就理解了
